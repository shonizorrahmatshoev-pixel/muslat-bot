...
async def start_command(update: Update, context: CallbackContext):
    msg = "**Welcome to Muslat Express**\n\n" \
          "**How to track**:\n" \
          "1 Click Contact button to register automatically\n" \
          "2 Send your tracking number directly\n\n" \
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if not check_user_registered(update.message.from_user.id):
        await update.message.reply_text(
            "Please register first! Click Contact button below OR type /register.",
            reply_markup=create_menu_keyboard()
        )
        return
...
async def list_shipments_cmd(update: Update, context: CallbackContext):
    ...
    if not shipments:
        await update.message.reply_text(
            "Database is empty. Use /addshipment to add records.",
            reply_markup=create_menu_keyboard()
        )
        return
